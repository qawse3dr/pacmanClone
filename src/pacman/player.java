...
public class player extends Gameobjects{
	...
	public player(int x,int y){
		...
		try {
	        ...
	        waka = AudioSystem.getAudioInputStream(getClass().getResource("sounds/waka.wav"));
	        clip.open(waka);
	      } catch (Exception e) {
		...
		this.y = y+3*16;
		speed = 4;
		availableMoves= new movement[]{movement.left,movement.right,movement.up,movement.down};
		spriteSheetX=0;
		spriteSheetY=0;
		this.fullRect = new Rectangle(x,y,16,16);
		this.rect = new Rectangle(x,y,16,16);
		try {
			this.image = ImageIO.read(getClass().getResource("images/pacmanSpriteSheet.png"));
		} catch (IOException e) {
	...
	public void draw(Graphics g) {
		g.drawImage(image, (int)x-6, (int)y-6, (int)x+22, (int)y+22, spriteSheetX*32, spriteSheetY*32, spriteSheetX*32+31, spriteSheetY*32+31, null);
		if (lives >0){
			g.drawImage(image, 32,34*16 ,64 ,34*16+32 ,32,32, 64, 64, null);
		}
		if (lives >1){
			g.drawImage(image, 64,34*16 ,64+32 ,34*16+32 ,32,32, 64, 64, null);
		}
	...
	public void tick() {
		...
		if (currentMove == movement.up){opMove = movement.down;}else if(currentMove == movement.none){opMove = movement.none;}else if(currentMove == movement.down){opMove = movement.down;}
		else if (currentMove == movement.left){opMove = movement.right;}else{opMove = movement.left;}
		availableMoves = new movement[]{currentMove,opMove};//directions you can move
		for (int pointIndex=0; pointIndex<turningPoint.turningPoints.size();pointIndex++){//finds if you can go any other directions
			...
			if (rect.contains(turningPoint.turningPoints.get(pointIndex).rect)){
				...
				availableMoves = turningPoint.turningPoints.get(pointIndex).availableMoves;
		...
		for (int ghostIndex=0;ghostIndex<4;ghostIndex++){
			if(ghost.ghosts[ghostIndex].rect.intersects(this.rect)){
				...
				pacman.score+=200;}else if(ghost.ghosts[ghostIndex].dead){
					continue;
				}else{
