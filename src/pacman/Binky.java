...
public class Binky extends ghost {
  ...
  public void draw(Graphics g) {
    currentSprite(this);
    g.drawImage(ghost.ghostSpriteSheet, (int)x - 5, (int)y - 5, (int)x + 22,
  ...
  public void tick() {
    ...
    this.rect.x = (int)x; // setting the new rect x and y
    this.rect.y = (int)y;
    if (start) {
      // making sides work for the Binky;

      if (dead && new Rectangle(11 * 16, 16 * 16, 6 * 16, 32).contains(rect)) {
        dead = false;
        isBlue = false;
      } else if (dead) {
        targetX = 15 * 16;
